...
fn test_completing_recurrence_workflow() {
    ...
    assert_eq!(
        actions.len(),
        2,
        "Should have 2 actions now (template + log entry)"
    );
    ...
    assert_eq!(
        ...
        "Template should be reset to NotStarted"
    );
    assert_eq!(
        template
            .do_date_time
            ...
            .to_string(),
        "2025-01-02",
        "Template should advance to next day"
    );
    assert!(
        template.recurrence.is_some(),
        "Template should still have recurrence rule"
    );
    ...
    assert_eq!(
        log_entry
            ...
            .to_string(),
        "2025-01-01",
        "Log entry should keep the original instance date"
    );
    assert!(
        log_entry.recurrence.is_none(),
        "Log entry should NOT have a recurrence rule"
    );
}
