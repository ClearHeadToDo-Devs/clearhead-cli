...
fn test_import_plans_splits_multi_event_ics_into_vdir_files() {
    ...
    env.command()
        .arg("import").arg("plans").arg(&source)
        .assert().success()
...
fn test_import_plans_honors_explicit_charter_flag() {
    ...
    env.command()
        .arg("import").arg("plans").arg(&source)
        .arg("--charter").arg("inbox")
        .assert().success()
        .stdout(predicate::str::contains("Imported 1 plan(s) into charter 'inbox'"));
    assert!(env.data_dir.join("plans").join("inbox").join("focus@example.com.ics").exists());
}

#[test]
fn test_import_plans_errors_on_existing_uid_without_overwrite() {
    let env = TestEnv::new();
    let existing = env.data_dir.join("plans").join("inbox").join("focus@example.com.ics");
    ...
    assert!(fs::read_to_string(existing).unwrap().contains("SUMMARY:Old Focus"));
}
...
fn test_import_plans_overwrites_existing_uid_with_flag() {
    ...
    env.command()
        .arg("import").arg("plans").arg(&source)
        .arg("--charter").arg("inbox").arg("--overwrite")
        .assert().success()
        .stdout(predicate::str::contains("(1 overwritten)"));
    assert!(fs::read_to_string(existing).unwrap().contains("SUMMARY:New Focus"));
}
...
fn test_error_on_missing_ics_file() {
    ...
    env.command()
        .arg("read").arg("plans")
        .arg("--file").arg("/nonexistent/path.ics")
        .assert().failure();
}
...
fn test_add_command_with_options() {
    let env = TestEnv::new();
    env.command()
        .arg("add").arg("plan").arg("High Priority Task")
        .arg("--priority").arg("1")
        .arg("--context").arg("work")
        .arg("--context").arg("urgent")
        .arg("--description").arg("Do it now")
        .arg("--rrule").arg("FREQ=WEEKLY;BYDAY=MO")
        .assert().success();
...
fn test_add_plan_file_flag_writes_single_event_file_to_explicit_path() {
    let env = TestEnv::new();
    let output = env.data_dir.join("plans").join("focus").join("focus-block.ics");
    env.command()
...
fn test_complete_plan_explains_state_lives_on_acts() {
    ...
    env.command()
        .arg("complete").arg("plan").arg("Scheduled Plan")
        .assert().failure()
...
fn test_sync_events_command() {
    ...
    env.command().arg("sync").arg("events")
        .assert().success()
...
fn test_read_plans_charter_filter() {
    ...
    env.command()
        .arg("read").arg("plans")
        .arg("--charter").arg("build_clearhead")
        .assert().success()
...
fn test_recursive_requires_charter() {
    ...
    env.command()
        .arg("read").arg("plans").arg("--recursive")
        .assert().failure();
}
