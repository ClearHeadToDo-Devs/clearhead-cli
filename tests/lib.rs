...
fn convert_basic_action_from_examples() {
    let test_action = get_test_examples().unwrap()["children"]["minimal"]["content"].clone();
    let test_config_str = r#"{}"#;
    let test_config = serde_json::from_str(test_config_str).expect("unable to conver");
...
fn get_test_examples() -> Result<HashMap<String, HashMap<String, HashMap<String, String>>>, String>
{
    ...
    let list_get_response = request_client.get("https://raw.githubusercontent.com/ClearHeadToDo-Devs/tree-sitter-actions/refs/heads/master/test/test_descriptions.json").send().expect("unable to get url in question").text().expect("unable to get text from request");
    let binding: serde_json::Value =
        serde_json::from_str(&list_get_response).expect("could not parse JSON");
    ...
    for (filename, tests) in test_list {
        ...
        for (test_name, description) in tests.as_object().expect("not a child object") {
            ...
            let test_file_contents = request_client.get(format!("https://raw.githubusercontent.com/ClearHeadToDo-Devs/tree-sitter-actions/refs/heads/master/examples/{}.actions",test_name)).send().expect("couldnt find example file").text().expect("unable to translate the body");
            file_tests.insert(
                test_name.to_string(),
                HashMap::from([
